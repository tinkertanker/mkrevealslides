    ...
    fn validate(&self) -> Result<(), AppError> {
        ...
        if !self.output_file.is_absolute() {
            return Err(AppError::from(
                ArgumentError::new("output_file".to_string(),
        ...
        if self.output_file.is_file() {
            ...
            warn!("Output file at `{}` already exists, will overwrite", self.output_file.display());
        } else {
        ...
        if !self.template_file.is_absolute() {
            return Err(AppError::from(
                ArgumentError::new("template_file".to_string(),
        ...
        for include_file in &self.include_files {
            if !include_file.is_absolute() {
                return Err(AppError::from(
                    ArgumentError::new("include_files".to_string(),
...
impl TryFrom<CliArgs> for PresentationConfig {
    ...
    fn try_from(args: CliArgs) -> Result<Self, Self::Error> {
        match args.command {
            ...
            Commands::FromCli {
                title, slide_dir,
                template_file, output_dir, output_file
            } => {
...
impl TryFrom<PresentationConfigFile> for PresentationConfig {
    ...
    fn try_from(config: PresentationConfigFile) -> Result<Self, Self::Error> {
        ...
        let include_files_abs_paths = config.include_files
            .iter()
            .map(|relative_pth| {
                config.working_directory.join(relative_pth)
            })
            .collect::<Vec<PathBuf>>();
        trace!("Converted {} include_file paths to abs paths", include_files_abs_paths.len());
